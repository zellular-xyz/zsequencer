...
AGGREGATE_PUBLIC_KEY_HEX: str = "a95b5b00610160521fc0a34bf5bc3e9c4e4b81ca10a99731de2291ad34f07224e16581c195d1452dfd75876c973853a1"
aggregated_pubkey: G1Element = G1Element.from_bytes(
    bytes.fromhex(AGGREGATE_PUBLIC_KEY_HEX)
)
...
if actual_signers < required_signers:
    raise ValueError(
        f"Not enough valid signatures! Required: {required_signers}, Got: {actual_signers}"
    )

# --- Subtract non-signing public keys from aggregate ---
# -- start: subtracting nonsigners --
for node_id in non_signing_nodes:
    node_pubkey: G1Element = G1Element.from_bytes(
        bytes.fromhex(NODE_CONFIG[node_id]["pubkey"])
    ).negate()
