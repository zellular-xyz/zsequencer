...
from sequencer_sabotage_simulation.schema import (NodeSimulationStateItem,
                                                  NodeSabotageSimulationState,
                                                  NetworkData)


def get_node_sabotage_config(config_file_path: str, address: str) -> NodeSabotageSimulationState:
...
def _validate_item(item: dict) -> NodeSimulationStateItem:
    ...
    if "time_duration" not in item or not isinstance(item["time_duration"], (int, float)):
