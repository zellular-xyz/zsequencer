...
async def _switch_sequencer_core(old_sequencer_id: str, new_sequencer_id: str):
    ...
    async with switch_lock:
        ...
        try:
            ...
            all_nodes_last_finalized_batch_records = (
                await get_network_last_finalized_batch_records()
            )
...
async def _fetch_node_last_finalized_batch_records_or_empty(
    session: aiohttp.ClientSession, node: dict[str, Any]
    ...
    try:
        async with session.get(
            ...
            if response.status != 200:
                ...
                zlogger.warning(
                    f"Failed to fetch last finalized record from node {node['socket']}: {error_text}"
                )
                return {}
            ...
            if data.get("status") == "error":
                zlogger.warning(
                    f"Failed to fetch last finalized record from node {node['socket']}: {data}"
                )
                return {}
        ...
        zlogger.warning(
            f"Failed to fetch last finalized record from node {node['socket']}: {e}"
        )
        return {}
...
async def get_network_last_finalized_batch_records() -> dict[str, BatchRecord]:
    ...
    highest_indices = {
        app_name: record.get("index", 0) for app_name, record in highest_records.items()
    }

    # Filter nodes to exclude self
    nodes_to_query = [
        node for node in zconfig.NODES.values() if node["id"] != zconfig.NODE["id"]
    ]
