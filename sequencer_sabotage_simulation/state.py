...
class SequencerSabotageSimulationState:
    ...
    def __init__(self, conf: OutOfReachSimulation, address: str):
        ...
        self._out_of_reach = False if len(self._sabotage_time_series_conf) == 0 else (
            not self._sabotage_time_series_conf[0]['up'])
    ...
    def start_simulating(self):
        ...
        self._monitor_thread = threading.Thread(target=self._simulate_out_of_reach, daemon=True)
...
sequencer_sabotage_simulation_state = SequencerSabotageSimulationState.get_instance(conf=OutOfReachSimulation(),
